...
internal class Examples
{
    ...
    public void ShouldXXXXXXXXXXXXX6() //bug
    {
        ...
        maybe.WhereValueExist().Should().Equal(new[] { 1, 2 });
        maybe.SelectWhereValueExist(n => n + 1).Should().Equal(new[] { 2, 3 });
        
        new[] { "1", "2", "2" }.FirstMaybe().Should().Be("1".Just());
        new[] { null, "2", "2" }.FirstMaybe().Should().Be(Maybe<string>.Nothing);
        new string[] { }.FirstMaybe().Should().Be(Maybe<string>.Nothing);

        new[] { "1", "2", "2" }.SingleMaybe().Should().Be(Maybe<string>.Nothing); //!
        new[] { null, "2", "2" }.SingleMaybe().Should().Be(Maybe<string>.Nothing);
        new string[] { }.SingleMaybe().Should().Be(Maybe<string>.Nothing);
    }
    
    [Test]
    public void ShouldXXXXXXXXXXXXX7() //bug
    {
        List<List<List<string?>>> nested = new List<List<List<string?>>>
        {
        ...
        List<List<List<string?>>> nested2 = new List<List<List<string?>>>();
